...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    m_outputSize.Width = DX::ConvertDipsToPixels(m_logicalSize.Width / m_compositionScaleX, m_dpi);
    m_outputSize.Height = DX::ConvertDipsToPixels(m_logicalSize.Height / m_compositionScaleY, m_dpi);
    ...
    else
    {
        ...
        m_swapChainPanel.get().Dispatcher().RunAsync(CoreDispatcherPriority::High, [=]()
            {
                auto panelNative{ m_swapChainPanel.get().as<ISwapChainPanelNative>() };
                winrt::check_hresult(
...
void DX::DeviceResources::SetSwapChain(winrt::Windows::UI::Xaml::Controls::SwapChainPanel const& panel)
{
    DisplayInformation currentDisplayInformation = DisplayInformation::GetForCurrentView();
    
    m_swapChainPanel = panel;
    m_logicalSize = winrt::Windows::Foundation::Size(panel.ActualWidth(), panel.ActualHeight());
    m_nativeOrientation = currentDisplayInformation.NativeOrientation();
    m_currentOrientation = currentDisplayInformation.CurrentOrientation();
    m_dpi = currentDisplayInformation.LogicalDpi();
    m_d2dContext->SetDpi(m_dpi, m_dpi);
